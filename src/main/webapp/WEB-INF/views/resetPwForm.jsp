...
<h2 class="text-center" style="font-family: 'Do Hyeon', sans-serif; font-size: 35px;">비밀번호를 재설정 해주세요.</h2>
<div class="login-form">
    <form action="/javas/resetPw" method="post">   
        <div class="form-group">
        	<div class="input-group">
                <span class="input-group-addon"><i class="fa fa-user"></i></span>
                <input type="password" onKeyUp="checkPW();" id="firstPW"
                class="form-control" name="password" placeholder="비밀번호" required="required">				
            </div>
        </div>
		<div class="form-group">
            <div class="input-group">
                <span class="input-group-addon"><i class="fa fa-lock"></i></span>
                <input type="password" onKeyUp="checkPW();" id="secondPW"
                class="form-control" name="passwordCheck" placeholder="비밀번호 확인" required="required">
                <input type="hidden" name="email" value="${email}" id="userEmail">		
            </div>
        </div>        
        <div class="form-group">
            <button id="resetBtn" type="button" onClick="resetPW();" class="btn btn-primary login-btn btn-block">비밀번호 재설정</button>
        </div>
    </form>
    <p id="checkMsg"></p>
</div>
...
function resetPW(){
	...
	var pw = document.getElementById("firstPW").value;
	var param = "email="+email+"&password="+pw;
	if(color=="green"){
		...
		request.onload = function(){
			if(request.status==200){
				...
				if(str=="success"){
					alert("비밀번호를 재설정했습니다. 로그인 후 서비스를 이용해주세요.");
				}
				else{
					alert("오류가 발생했습니다. 다시 시도해주세요.");
				}
				document.location.href = "/javas/login/form";
			}
		}
		request.open("POST","/javas/resetPw",true);
